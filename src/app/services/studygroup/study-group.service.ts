...
export class StudyGroupService {
  ...
  async createOrFindStudyGroup(studyGroupName: string, ID: string): Promise<StudyGroup[]> {
    let studygroup = await this.afs.collection<StudyGroup>('studygroups', ref => ref.where('name', '==', studyGroupName)).valueChanges()
    .pipe(
      first()
    ).toPromise();
    ...
    if (studygroup.length === 0) {
      ...
      studygroup = await this.afs.collection<StudyGroup>('studygroups', ref =>
      ref.where('studyGroupName', '==', studyGroupName)).valueChanges()
      .pipe(
        first()
      ).toPromise();
    }
